...
/**
 * Dialog for creating a new dataset from a TSV file.
 * 
 * @author niko.popitsch
 *
 */
public class GIELoadTSVDialog extends JDialog {
    ...
    public GIELoadTSVDialog(Frame owner, File f) throws IOException {
	super(owner, "New Dataset From TSV", true);
	this.file = f;
	init();
    }
	...
	if (coords != null)
	    GIE.getInstance().getWindowCoordinates().put("GIELoadTSVDialog", getCoords());
    }
	...
	// ======== this ========
	setTitle("VARAN-GIE :: Create New Dataset From TSV");
	...
	Integer[] coords = GIE.getInstance().getWindowCoordinates().get("GIELoadTSVDialog");
	if (coords == null) {
	...
	AutoCompletionListener autoComplete = new AutoCompletionListener(textField0, GIE.getInstance().getCategories());
	...
	SpringUtilities.makeCompactGrid(formPanel, 12, 2, // rows, cols
		6, 6, // initX, initY
		6, 6); // xPad, yPad
	...
	button1.addActionListener(new ActionListener() {
		...
		try {
			...
			RegionOfInterest roi = new RegionOfInterest(chr, start, end, name==null?"":(name.equals(NONE) ? "" : name));
			...
			for (String c : customFields)
			    roi.addAnnotation(c, URLDecoder.decode(ti.getField(t, col2fieldMap.get(c)), "UTF-8"));
			regions.add(roi);
		    }
		    ok = true;
		} catch (Exception e1) {
		...
		if (!ok) {
		    JOptionPane.showMessageDialog(IGV.getMainFrame(),
			    "Could not create new dataset '" + dsname
		...
		// create dataset
		try {
		    ok = GIE.getInstance().addDataset(category, dsname, description, customFields, null);
		...
		GIE.getInstance().loadDataset(dsname, null); // will automatically select latest created version.
